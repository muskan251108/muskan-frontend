...
function PatientDashboard() {
  ...
  const t = {
    hindi: {
      ...
      logout: "Logout", settings: "Settings", profile: "Profile", home: "Home",
      darkMode: "Dark Mode", language: "Language",
      profileTitle: "Meri Profile", name: "Naam", email: "Email", role: "Patient",
      doctor: "Doctor", status: "Status", time: "Time", date: "Date",
    },
    english: {
      ...
      logout: "Logout", settings: "Settings", profile: "Profile", home: "Home",
      darkMode: "Dark Mode", language: "Language",
      profileTitle: "My Profile", name: "Name", email: "Email", role: "Patient",
      doctor: "Doctor", status: "Status", time: "Time", date: "Date",
    }
  };
  const tx = t[language];
  ...
  const inp = { width:"100%", padding:"11px 14px", borderRadius:"10px", border: inputBorder, fontSize:"15px", marginBottom:"16px", outline:"none", boxSizing:"border-box", background: inputBg, color: inputColor };
    ...
    <div style={{ minHeight:"100vh", background: bg, fontFamily:"'Segoe UI', sans-serif", transition:"background 0.3s" }}>
      ...
      <nav style={{ background: navBg, borderBottom: border, padding:"0 32px", height:"64px", display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:100, backdropFilter:"blur(10px)" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:"12px" }}>
          <span style={{ fontSize:"14px", fontWeight:"600", color: textSecondary }}>🤒 {profile?.name || "Patient"}</span>
          ...
          <>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))", gap:"16px", marginBottom:"24px" }}>
              {[
                ...
                <div key={label} style={{ background: cardBg, borderRadius:"14px", padding:"20px 24px", border, borderTop:`3px solid ${color}`, transition:"background 0.3s" }}>
                  ...
                  <div style={{ fontSize: typeof val==="string"&&val.length>6?"15px":"24px", fontWeight:"800", color: textPrimary }}>{val}</div>
                </div>
              ))}
            </div>
            ...
            {assigned && (
              <div style={{ background: cardBg, borderRadius:"14px", padding:"28px", border, marginBottom:"20px", transition:"background 0.3s" }}>
                <div style={{ fontSize:"17px", fontWeight:"700", color: textPrimary, marginBottom:"16px", paddingBottom:"14px", borderBottom: border }}>⭐ {tx.assignedTitle}</div>
                <div style={{ background:"rgba(0,168,255,0.08)", border:"1.5px solid rgba(0,168,255,0.3)", borderRadius:"12px", padding:"18px 22px", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:"12px" }}>
                  <div>
                    <div style={{ fontWeight:"700", fontSize:"17px", color: textPrimary }}>Dr. {assigned.name}</div>
                  ...
                  <button onClick={() => openModal(assigned)}
                    style={{ padding:"10px 22px", borderRadius:"9px", border:"none", background:"linear-gradient(135deg,#00a8ff,#0057ff)", color:"#fff", fontWeight:"600", fontSize:"14px", cursor:"pointer" }}>
                    📅 {tx.bookKaro}
                  </button>
                </div>
              </div>
            ...
            {/* All Doctors */}
            <div style={{ background: cardBg, borderRadius:"14px", padding:"28px", border, marginBottom:"20px", transition:"background 0.3s" }}>
              <div style={{ fontSize:"17px", fontWeight:"700", color: textPrimary, marginBottom:"20px", paddingBottom:"14px", borderBottom: border }}>{tx.allDoctors}</div>
              {loading ? <div style={{ textAlign:"center", color: textSecondary, padding:"32px 0" }}>{tx.loading}</div>
              : doctors.length === 0 ? <div style={{ textAlign:"center", color: textSecondary, padding:"32px 0" }}>{tx.noDoctor}</div>
              : doctors.map(doc => (
                <div key={doc._id} style={{ border, borderRadius:"12px", padding:"16px 20px", marginBottom:"12px", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:"12px", background: darkMode?"rgba(255,255,255,0.03)":"#f8fafc" }}>
                  <div>
                  ...
                  <button disabled={!doc.available} onClick={() => doc.available && openModal(doc)}
                    style={{ padding:"9px 22px", borderRadius:"9px", border:"none", background: doc.available?"linear-gradient(135deg,#00a8ff,#0057ff)":"rgba(255,255,255,0.08)", color: doc.available?"#fff": textSecondary, fontWeight:"600", fontSize:"14px", cursor: doc.available?"pointer":"not-allowed" }}>
            ...
            {/* My Appointments */}
            <div style={{ background: cardBg, borderRadius:"14px", padding:"28px", border, transition:"background 0.3s" }}>
              <div style={{ fontSize:"17px", fontWeight:"700", color: textPrimary, marginBottom:"20px", paddingBottom:"14px", borderBottom: border }}>{tx.myApptTitle}</div>
              {myAppts.length === 0 ? <div style={{ textAlign:"center", color: textSecondary, padding:"32px 0" }}>{tx.noAppt}</div>
              : (
                <table style={{ width:"100%", borderCollapse:"collapse" }}>
                  <thead>
                    <tr>{[tx.doctor, tx.date, tx.time, tx.status].map(h=>(
                  ...
                  <tbody>
                    {myAppts.map(a => (
                      <tr key={a._id}>
                        <td style={{ padding:"14px", fontSize:"14px", color: textPrimary, borderBottom: tableBorder }}>Dr. {a.doctorName || "—"}</td>
        ...
        {activePage === "profile" && (
          <div style={{ maxWidth:"500px", margin:"0 auto" }}>
            ...
            <div style={{ background: cardBg, borderRadius:"20px", padding:"36px", border, textAlign:"center", transition:"background 0.3s" }}>
              <div style={{ width:"80px", height:"80px", borderRadius:"50%", background:"linear-gradient(135deg,#48bb78,#276749)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"32px", margin:"0 auto 20px" }}>
                🤒
              </div>
              <div style={{ fontSize:"22px", fontWeight:"700", color: textPrimary, marginBottom:"6px" }}>{profile?.name}</div>
              <div style={{ fontSize:"14px", color:"#48bb78", marginBottom:"24px" }}>{tx.role}</div>
      ...
      {modal.open && (
        <div style={{ position:"fixed", top:0, left:0, right:0, bottom:0, background:"rgba(0,0,0,0.7)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:999 }}>
          <div style={{ background: darkMode?"#162033":"#ffffff", borderRadius:"20px", padding:"36px", width:"100%", maxWidth:"400px", boxShadow:"0 20px 60px rgba(0,0,0,0.5)", margin:"16px", border }}>
            <div style={{ fontSize:"18px", fontWeight:"700", color: textPrimary, marginBottom:"4px" }}>{tx.bookAppt}</div>
            <div style={{ fontSize:"13px", color: textSecondary, marginBottom:"24px" }}>Dr. {modal.doctor?.name} • ₹{modal.doctor?.fees} • {modal.doctor?.timing}</div>
